#!/usr/bin/env python3
"""Normalize CDDIS PRIDE kin_* outputs into the plotting export format."""
...
SOURCE = {
    ...
    "region": "Global CDDIS",
    "network": "NASA CDDIS",
    "source_label": "NASA CDDIS PRIDE PPP-AR kin quality-passing stations",
}
...
def resolve_workflow_path(value: str | Path, summary: dict) -> Path:
...
def read_event(conn: sqlite3.Connection, event_id: str, fallback_time: str) -> dict:
    ...
    if row:
        event = dict(row)
        event["time_utc"] = event.pop("event_time_utc")
        event["event_date"] = event["time_utc"][:10]
        event["title"] = event.get("place") or event_id
        event["depth_km"] = None
        event["usgs_url"] = ""
        return event
    return {
...
def read_station_metadata(conn: sqlite3.Connection, event_id: str) -> dict[str, dict[str, float]]:
    ...
    rows = conn.execute(
        """
        SELECT station4, station_latitude, station_longitude, MIN(distance_km) AS distance_km
    ...
    for row in rows:
        station = str(row["station4"]).upper()
        metadata[station] = {
...
def event_grade(station_rows: list[dict], azimuth_bin_count: int = 8) -> dict[str, object]:
    ...
    if station_count >= 8 and covered_bins >= 6:
        ...
        grade = "C"
        description = "single/few-station event retained for CDDIS prototype"
    else:
        grade = "D"
...
def normalization_metadata(include_warn: bool) -> dict[str, object]:
    return {
        ...
        "collection_policy": "CDDIS prototype export; retain any event with at least one quality-passing station",
    }
...
def workflow_event_fields(summary: dict) -> tuple[str, str]:
    event_id = str(summary.get("event_id") or "").strip()
    event_time = str(summary.get("process_event_time_utc") or summary.get("event_time_utc") or "").strip()
    if not event_id or not event_time:
        raise SystemExit("Workflow summary is missing event_id or process_event_time_utc")
...
def workflow_kin_files(summary: dict, workflow_summary: Path) -> list[Path]:
    ...
    if manifest.exists():
        return unique_paths(
            [
                resolve_workflow_path(line.strip(), summary)
                for line in manifest.read_text(encoding="utf-8").splitlines()
                if line.strip()
            ]
        )
    run_root = summary.get("paths", {}).get("run_root")
    if run_root:
        return sorted(resolve_workflow_path(run_root, summary).rglob("kin_*"))
    return []


def write_outputs(args: argparse.Namespace, summary: dict, quality: dict) -> dict:
    event_id, event_time_text = workflow_event_fields(summary)
    ...
    (output_dir / "event.json").write_text(json.dumps(event_json(event, len(station_rows), args.workflow_summary, grade, args.include_warn), indent=2, ensure_ascii=False) + "\n", encoding="utf-8")
    ...
    provenance = {
        ...
        "notes": "Generated from CDDIS prototype PRIDE kin_* files; outputs are isolated from the standard EarthScope/GeoNet normalized roots.",
        ...
        "event_id": event_id,
        "event_time_source": "workflow_summary.process_event_time_utc",
